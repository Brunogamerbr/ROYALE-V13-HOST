...
let db = await database.ref(`Banco/${user.id}`).once('value');
  ...
  let dbnref = database.ref(`Economia/${user.id}`);
    ...
    if (!args[0]) {
        return message.inlineReply(`<:erro:858615784771551252> **|** Coloque o valor do saque!`);
    };
    ...
    if (db.val().bank < args[0]) {
      message.inlineReply(`<:erro:858615784771551252> **|** Você não Dinheiro no Banco o suficiente para realizar o saque!`);
    return;
    }
    
    if(args[0] < 0) {
        return message.inlineReply(`<:erro:858615784771551252> **|** Você tem que colocar um valor maior que **0** para realizar o saque!`);
    };
...
if (db.val().bank == 0) {
  message.inlineReply(`Você não tem saldo disponível na sua **Carteira**!`)
return;
}
...
if (isNaN(args[0])){
        return message.inlineReply(`<:erro:858615784771551252> **|** Você tem que colocar um valor numerico para realizar o saque!`);
    };
