...
c1.on("collect", async (m, u) => {
  ...
  switch(m.emoji.id) {
    case `910846323463422003`:
      if(db.val().dinheiro < 7000) return message.inlineReply(`:x: **|** Você não tem dinheiro suficiente para comprar um **Porte de Armas**`);

message.inlineReply(`Você comprou um porte de armas!`);
    ...
    message.inlineReply(`Escolta em manutenção!`)
      return;
      break;
    case "910848201672773642":
    
    
      if(db.val().dinheiro < 5000) return message.inlineReply(`:x: **|** Você não tem dinheiro suficiente para comprar uma **Pistola**`);
      message.inlineReply(`Você comprou uma pistola!`);
    ...
    case "910843230273282058":
      ...
      message.inlineReply(`MisteryBox em manutenção!`)
    break;
  case "910846793749758012":
      if (db.val().dinheiro <500) {
message.inlineReply(`**<:erro:858615784771551252>| Você não tem dinheiro na carteira suficiente para comprar um Roll!**`)
return;       
}
      message.inlineReply(`Você comprou **1** Roll no valor de **R$500**!`)
