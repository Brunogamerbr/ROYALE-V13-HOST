...
database.ref(`Start/${user.id}`).once('value');
  ...
  let db3 = await   
database.ref(`Banco/${user.id}`).once('value');
  let db3ref = database.ref(`Banco/${user.id}`);

  let dbL = await   
database.ref(`Loja/${user.id}`).once('value');
  let dbLref = database.ref(`Loja/${user.id}`);

  let db4 = await   
database.ref(`Empregos/${user.id}`).once('value');
  let db4ref = database.ref(`Empregos/${user.id}`);

let db6 = await   
database.ref(`Work/${user.id}`).once('value');
...
if (db.val() == null) {
  ...
  const msg = await message.inlineReply(`**${ayy} Trazendo suas informações aguarde..**`)
