...
database.ref(`VersaoBuild`).once('value');
  ...
  if (dbs.val() == null) {
  message.inlineReply(`**Antes de começar  usar minha Economia você deve usar \`${prefix}start\` Para liberar meus comandos Economia**`)
return;
    }
...
if(!args[0]) {
  return message.inlineReply(`**<:erro:858615784771551252> | Coloque o valor do depósito, ou use \` all \` para depoistar tudo!**`);
}

let money = (args[0].toLowerCase() == "all") ? dbE.val().dinheiro : parseInt(args[0]);

if(dbE.val().dinheiro < 1) {
  return message.inlineReply(`**<:erro:858615784771551252> | Você não tem dinheiro em sua carteira para fazer um deposito!**`)
}
...
let embed9 = new Discord.MessageEmbed()
    ...
    .setDescription(`Você depositou um valor de **R$${cacheAbb(money)}** 
Em sua conta **Bancária** Agora seu dinheiro está em **Segurança**.`)
