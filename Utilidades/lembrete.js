...
exports.run = async (client, message, args, database) => {
        ...
        let time = args[0]
        if(!time) return message.inlineReply("**Para definir um lembrete Use o comando dessa Forma: \`.lembrete [tempo] [mensagem de lembrete]\`**")
        let lembrete = args.slice(1).join(" ")
        if(!lembrete) return message.inlineReply(`**Por favor. Defina um texto lembrete**`)
