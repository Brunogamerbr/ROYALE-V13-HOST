...
exports.run = async (client, message, args, database) => {
    ...
    let quantia = args[0];
if (!quantia){
  return message.inlineReply(`<:erro:858615784771551252>| Você não colocou o número de mensagens para ser apagadas!`)
}
...
if (!message.member.permissions.has("MANAGE_MESSAGES")) return message.channel.send(`<:erro:858615784771551252>| Você não possui permissão para usar esse comando!`);
  ...
  let tanto = 100;
  if (quantia >tanto){
      return message.inlineReply(`<:erro:858615784771551252>| Eu só consigo apagar de 0 a 100 mensagens..`)
  }
